...
def dns_query(type, name, server):
    ...
    ARCOUNT = 0
        ...
        # Example: The QR field is located in the second byte of the DNS header, with its most significant bit being the leftmost bit of this byte.
        # Since each byte contains 8 bits, shifting the value of the QR field left by 15 bits moves it to the correct position in the 16-bit value
        # that represents the combination of several fields in the DNS header.
    ...
    ID, FLAGS, QDCOUNT, ANCOUNT, NSCOUNT, ARCOUNT = struct.unpack('!HHHHHH', response_header) # We are unpacking the binary data of the response header into individual values representing the
    #fields of the DNS header.
    
    # Parse the response question section (same as query)
    response_question = data[12:12+len(question)] # The data variable starts immediately after the header section, so what is it's index?
    #Note the two '??' '??' will be the same value as we start at a specific index and then go for the entire length of the binary data received. 
    ...
    for _ in range(ANCOUNT):
        ...
        type, cls, ttl, rdlength = struct.unpack('!HHIH', response_answer[offset:offset+11]) # What is the offset value in bytes? Remember 'H' represent 2 bytes,
        # and 'I' represents 4 bytes, we declared '!HHIH'. 
        
        offset += 11 # Same value as just calculated
